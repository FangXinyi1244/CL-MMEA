...
def demo_inference(model_path, file_dir, cuda=False):
    ...
    # 定义 action="store_false" 的参数
    store_false_flags = {'w_gcn', 'w_rel', 'w_attr', 'w_name', 'w_char', 'w_img'}
    
    # 构建命令行参数
    sys_module.argv = ['demo.py']
    for key, value in saved_args.items():
        if isinstance(value, bool):
            if key in store_false_flags:
                # action="store_false": 只有 value=False 时才添加参数
                if not value:
                    sys_module.argv.append(f'--{key}')
            else:
                # action="store_true": 只有 value=True 时才添加参数
                if value:
                    sys_module.argv.append(f'--{key}')
        else:
            sys_module.argv.append(f'--{key}={value}')
    
    # 创建 MCLEA 实例（现在会使用正确的参数）
    model = MCLEA()
    ...
    # ========== 确保 input_idx 已初始化 ==========
    if model.input_idx is None:
        model.input_idx = torch.LongTensor(np.arange(model.ENT_NUM)).to(model.device)
    # ========== 添加结束 ==========
